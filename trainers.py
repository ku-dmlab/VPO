...
class BasicTrainer(object):
    ...
    def sample(self, include_original_prompt=False) -> List[Dict[str, str]]:
        ...
        for eval_batch in self.eval_batches:
            ...
            all_best_chosen.extend(eval_batch['best_chosen'])
...
class PairedPreferenceTrainer(BasicTrainer):
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        if self.reference_model is None:
            ...
            ##########
            if self.config.loss['name'] in ['vdpo', 'vipo']:
                chosen = torch.tensor(batch['chosen_scores']).float()
                reject = torch.tensor(batch['rejected_scores']).float()
                if self.config.loss.exp:
...
class DPOTrainer(PairedPreferenceTrainer):
       def loss(self,
             ...
             p: torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """Compute the DPO loss for a batch of policy and reference model log probabilities."""
        pi_logratios = policy_chosen_logps - policy_rejected_logps
        ref_logratios = reference_chosen_logps - reference_rejected_logps
        logits = pi_logratios - ref_logratios

        losses = -F.logsigmoid(self.config.loss.beta * logits) * (1 - self.config.loss.label_smoothing) - F.logsigmoid(-self.config.loss.beta * logits) * self.config.loss.label_smoothing
...
class VDPOTrainer(PairedPreferenceTrainer):
       def loss(self,
             policy_chosen_logps: torch.FloatTensor,
             policy_rejected_logps: torch.FloatTensor,
             reference_chosen_logps: torch.FloatTensor,
        ...
        losses = forward_losses * p.to(policy_chosen_logps) + reverse_losses * (1 - p.to(policy_chosen_logps))
        chosen_rewards = self.config.loss.beta * (policy_chosen_logps - reference_chosen_logps).detach()
        rejected_rewards = self.config.loss.beta * (policy_rejected_logps - reference_rejected_logps).detach()
...
class RDPOTrainer(PairedPreferenceTrainer):
       def loss(self,
        ...
        forward_losses = -F.logsigmoid(self.config.loss.beta * ((policy_chosen_logps - reference_chosen_logps) - (policy_rejected_logps - reference_rejected_logps)))
        reverse_losses = -F.logsigmoid(self.config.loss.beta * ((policy_rejected_logps - reference_rejected_logps) - (policy_chosen_logps - reference_chosen_logps)))
        losses = (1 - self.config.loss.epsilon)/(1 - 2*self.config.loss.epsilon) * forward_losses + (-self.config.loss.epsilon)/(1 - 2*self.config.loss.epsilon) * reverse_losses
...
class VIPOTrainer(PairedPreferenceTrainer):
       def loss(self,
             policy_chosen_logps: torch.FloatTensor,
             policy_rejected_logps: torch.FloatTensor,
             reference_chosen_logps: torch.FloatTensor,
        ...
        losses = (logits - (2*p.to(policy_chosen_logps) -1)/(2 * self.config.loss.beta)) ** 2
        chosen_rewards = self.config.loss.beta * (policy_chosen_logps - reference_chosen_logps).detach()
        rejected_rewards = self.config.loss.beta * (policy_rejected_logps - reference_rejected_logps).detach()
